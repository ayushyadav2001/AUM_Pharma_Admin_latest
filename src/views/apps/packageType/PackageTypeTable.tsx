...
const PackageTypesTable = ({ tableData }: { tableData?: UsersType[] }) => {
  ...
  const handleDelete = useCallback(async () => {
    if (selectedItem) {
      try {
        await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}/package-type/delete-packaging/${selectedItem}`, { withCredentials: true });
  ...
  const handleStatusChange = useCallback(async () => {
    if (selectedItem) {
      try {
        await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_URL}/package-type/status-change-packaging/${selectedItem}`, {}, { withCredentials: true });
        await fetchPackagingType();
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-0.5'>
            ...
            <IconButton size='small'>
              <Link href={getLocalizedUrl(`/apps/packaging-type/${row.original._id}`, locale as Locale)} className='flex'>
      ...
      <Card>
        ...
        <CardContent className='flex justify-between flex-col items-start sm:flex-row sm:items-center max-sm:gap-4'>
          ...
          <div className='flex flex-col !items-start max-sm:is-full sm:flex-row sm:items-center gap-4'>
            ...
            <Link className='' href={`/${locale}/apps/packaging-type/add`}>

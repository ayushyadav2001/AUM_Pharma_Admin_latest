...
const TrendingTodayTable = ({ tableData }: { tableData?: UsersType[] }) => {
  // States
  ...
  const handleDelete = useCallback(async () => {
    if (selectedItem) {
      try {
        await axios.delete(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/trending-today/delete-trending-banner/${selectedItem}`,
          {
            withCredentials: true
          }
        )
  ...
  const handleStatusChange = useCallback(async () => {
    if (selectedItem) {
      try {
        await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/trending-today/change-status-trending-banner/${selectedItem}`,
  ...
  const columns = useMemo<ColumnDef<any>[]>(
    () => [
      ...
      columnHelper.accessor('banner', {
        header: 'Banner',
        cell: ({ row }) => {
          const bannerImage = row.original.banner

          const imageUrl = `${process.env.NEXT_PUBLIC_IMAGE_URL}${bannerImage}`
        ...
        cell: ({ row }) => (
          <div className='flex items-center  justify-center'>
            <Chip
              ...
              color={row.original.status === true ? 'success' : 'warning'}
              size='small'
            />
          </div>
        )
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-0.5'>
            ...
            <IconButton size='small'>
              <Link
                href={getLocalizedUrl(`/apps/trending-today/edit/${row.original._id}`, locale as Locale)}
                className='flex'
              >
  ...
  return (
    <Card>
      <CardHeader title='Trending Today' className='pbe-4' />
      ...
      <CardContent className='flex justify-between flex-col items-start sm:flex-row sm:items-center max-sm:gap-4'>
        ...
        <div className='flex flex-col !items-start max-sm:is-full sm:flex-row sm:items-center gap-4'>
          ...
          <Link className='' href={`/${locale}/apps/trending-today/add`}>
      ...
      <ConfirmationDialogStatusChange
        open={confirmationStatusDialogOpen}
...
export default TrendingTodayTable

  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/trending-today/add-trending-banner`,
        formData,
        {
          withCredentials: true,
          headers: {
      ...
      if (response.status === 201) {
        toast.success(`Banner Added Successfully! Message: ${response.data.message}`)
...
export default AddTrendingToday

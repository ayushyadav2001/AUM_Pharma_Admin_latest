...
const StockListTable = ({ tableData }: { tableData?: UsersType[] }) => {
  // States
  ...
  const columns = useMemo<ColumnDef<any, any>[]>(
    () => [
      ...
      columnHelper.accessor('product', {
        ...
        cell: ({ row }) => {
          const productImage = row.original.product.product_image;
      ...
      //   header: 'Category',
      //   cell: ({ row }) => <Typography>{row.original.product.category?.name || 'N/A'}</Typography>
      // }),
      // columnHelper.accessor('sub_category', {
      //   header: 'Sub Category',
      //   cell: ({ row }) => <Typography>{row.original.product.sub_category?.name || 'N/A'}</Typography>
      // }),
      columnHelper.accessor('product_id', {
        header: 'Product ID',
        cell: ({ row }) => <Typography>{row.original.product.product_id}</Typography>
      }),
      ...
      columnHelper.accessor('mrp', {
        header: 'MRP',
        cell: ({ row }) => <Typography>₹ {row.original.product.mrp}</Typography>
      }),
      columnHelper.accessor('manufacturer', {
        header: 'Manufacturer',
        cell: ({ row }) => <Typography>{row.original.product.manufacturer}</Typography>
      }),
      columnHelper.accessor('stock', {
        header: 'Current Stock',
        cell: ({ row }) => <Typography>{row.original.stock?.current_stock} pcs</Typography>
      }),
      columnHelper.accessor('stock', {
        header: 'Stock In',
        cell: ({ row }) => <Typography>{row.original.stock?.stock_in ? row.original.stock?.stock_in : 0} pcs</Typography>
      }),
      columnHelper.accessor('stock', {
        header: 'Stock Out',
        cell: ({ row }) => <Typography>{row.original.stock?.stock_out ? row.original.stock?.stock_out : 0} pcs</Typography>
      }),
  ...
  return (
    <>
      <Card >
        <CardHeader title='Stocks' className='pbe-4' />

...
const ManufacturerTable = ({ tableData }: { tableData?: UsersType[] }) => {
  // States
  ...
  const handleDelete = useCallback(async () => {
    if (selectedItem) {
      try {
        await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}/manufacturer/delete-manufacturer/${selectedItem}`, { withCredentials: true });
        await fetchPackagingType();
  ...
  const handleStatusChange = useCallback(async () => {
    if (selectedItem) {
      try {
        await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_URL}/manufacturer/status-change-manufacturer/${selectedItem}`, {}, { withCredentials: true });
        await fetchPackagingType();
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-0.5'>
            ...
            <IconButton size='small'>
              <Link href={getLocalizedUrl(`/apps/manufacturer/${row.original._id}`, locale as Locale)} className='flex'>
  ...
  return (
    <>
      <Card>
        <CardHeader title={`Manufacturer's`} className='pbe-4' />
        ...
        <CardContent className='flex justify-between flex-col items-start sm:flex-row sm:items-center max-sm:gap-4'>
          ...
          <div className='flex flex-col !items-start max-sm:is-full sm:flex-row sm:items-center gap-4'>
            ...
            <Link className='' href={`/${locale}/apps/manufacturer/add`}>

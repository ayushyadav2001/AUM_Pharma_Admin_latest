...
import ConfirmationDialog from './deleteStockAdjustmentModel';
...
const StockAdjustmentListTable = ({ tableData }: { tableData?: UsersType[] }) => {
  // States
  ...
  const fetchStocks = async () => {
    try {
      ...
      setStockAdjustmentData(response.data.data); // Update state with fetched data
    } catch (err) {
      console.error('Failed to fetch stock adjustments', err); // Log the error for debugging
    } finally {
  ...
  const handleDelete = useCallback(async () => {
    if (selectedItem) {
      try {
        await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}/stock-adjust/delete-adjustment/${selectedItem}`, { withCredentials: true });
        await fetchStocks(); // Refresh the list after deletion
      } catch (err) {
  ...
  const getMostRecentAdjustment = (adjustmentHistory: any) => {

    if (!adjustmentHistory || adjustmentHistory.length === 0) return null;

    const mostRecent = adjustmentHistory[adjustmentHistory.length - 1];

    return mostRecent.adjustedBy;
  };
  ...
  const productStatusObj: any = {
    inventory: { title: 'Inventory', color: 'default' },
    damage: { title: 'Damage', color: 'error' },
    loss: { title: 'Loss', color: 'warning' },
    sale: { title: 'Sale', color: 'success' },
    return: { title: 'Return', color: 'info' },
  ...
  const columns = useMemo<ColumnDef<any, any>[]>(
    () => [
      ...
      columnHelper.accessor('adjustmentDate', {
        header: 'Date',
        cell: ({ row }) => <Typography>{formattedDate(row.original.adjustmentDate)}</Typography>
      }),
      ...
      columnHelper.accessor('adjustmentType', {
        ...
        cell: ({ row }) =>
          <Chip
            label={row.original.adjustmentType == "increase" ? "Increase" : "Decrease" || 'Unknown'}
            variant='outlined'
            color={row.original.adjustmentType == "increase" ? "success" : "error" || 'default'}
            size='small'
          />
      ...
      columnHelper.accessor('quantity', {
        ...
        cell: ({ row }) => {
          ...
          return <Typography>{formatCurrency(row.original.totalAmount)}</Typography>;
        },
      }),
      columnHelper.accessor('totalAmountRecovered', {
        header: 'Amount Recovered',
        cell: ({ row }) => <Typography>{formatCurrency(row.original.totalAmountRecovered)}</Typography>
      }),
      columnHelper.accessor('reason', {
        header: 'Adjustment Type',
        cell: ({ row }) => {

          const status = row.original.adjustmentDue;

          const { title, color } = productStatusObj[status] || {};
      ...
      columnHelper.accessor('adjustmentHistory', {
        header: 'Added By',
        cell: ({ row }) => {
          const adjustedBy = getMostRecentAdjustment(row.original.adjustmentHistory);

          if (!adjustedBy) return <Typography>NA</Typography>;

          const { firstName, lastName } = adjustedBy;
      ...
      columnHelper.accessor('action', {
        ...
        cell: ({ row }: any) => (
          <div className='flex items-center gap-0.5'>
            <IconButton size='small' onClick={() => openConfirmationDialog(row.original._id)}>
              <i className='ri-delete-bin-7-line text-textSecondary' />
            </IconButton>
            <IconButton size='small'>
              <Link href={getLocalizedUrl(`/apps/stock-adjustment/view/${row.original?._id}`, locale as Locale)} className='flex'>
  ...
  return (
    <>
      <Card >
        <CardHeader title='Stocks Adjustments' className='pbe-4' />
        ...
        <div className='flex justify-between gap-4 p-4 flex-col items-start sm:flex-row sm:items-center'>
          ...
          <div className='flex items-center gap-x-4 max-sm:gap-y-4 is-full flex-col sm:is-auto sm:flex-row'>
            ...
            <Link className='hidden' href={`/${locale}/apps/products/add`}>
